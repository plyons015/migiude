...
| `MediaRecorder` ~8s chunks | `lib/speech/cloud-audio-capture.ts` |
...
3. Listen → start mic → lines appear every ~8s with **S1**, **S2**, … labels.
...
Uses the same **GEMINI_API_KEY** as other AI tasks. Blaze + Generative Language API billing applies per chunk (~every 8 seconds while listening).

## Limitations (v1)

- ~8 second latency per line (chunk boundary), not true streaming.
